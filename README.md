# Handoff: Sello Landing Pages

This package documents **two** linked HTML pages that share one design system:

1. **`Sello.html`** — the primary protocol landing page (documented first, below).
2. **`Sello - Find My for Agents.html`** — a concept/metaphor page that reframes Sello as "Find My, for AI agents" (documented in its own section near the end).

The two pages cross-link in their navs and CTAs ("Find My ↗" on the protocol page; "Protocol ↗" / "Back to the protocol" on the concept page). They are visually identical in system (same tokens, type, motion) — recreate them as two routes/pages in the same codebase.
...
- **Left column content:**
  ...
  - Pronunciation chip: pill (`border-radius: 999px`, `1px` soft border, `6px 14px`), mono 12.5px, `--ink-3`. Contents: `/SEH-yoh/ · Spanish for "seal" or "stamp"` (the gloss in accent color; slashes signal it's a pronunciation respelling).
...
- `Sello.html` — the primary protocol landing page (single self-contained file; all CSS in a `<style>` block in `<head>`, all JS in a `<script>` at the end of `<body>`). Source of truth for any detail not captured above.
- `Sello - Find My for Agents.html` — the concept/metaphor page (also self-contained; documented in the next section). Shares the exact same design tokens and base styles as `Sello.html`, plus an interactive Find My-style panel.

---

# Page 2: "Find My for AI Agents" (`Sello - Find My for Agents.html`)

## Overview

A concept/positioning page that explains Sello through an analogy: **Find My / AirTags, for AI agents.** The framing: an AirTag never trusts the bag it's in to report its location — the network around it does. Sello works the same way — every service an agent touches drops a sealed, verifiable "sighting" only the owner can decrypt. The page is intentionally a _metaphor_ layer that funnels to the real protocol page and the repo; copy leans into the analogy.

It **reuses the entire design system** from `Sello.html` (same OKLCH tokens, Space Grotesk / Hanken Grotesk / JetBrains Mono, verified-green accent, reveal-on-scroll, nav/footer patterns). One token is added: a supporting **cool blue** used for the "owner / encrypted / live" state — `--blue: oklch(0.80 0.10 200)` (~`#52c4d6`) and `--blue-dim: oklch(0.55 0.09 200)`.

## Page structure

1. **Nav** — same as protocol page; brand carries a small "Concept" tag; links: "The metaphor", "The network", "Privacy", "Protocol ↗" (→ `Sello.html`), GitHub button.
2. **Hero** — eyebrow "The mental model", H1 "Find My, for **AI agents**", lead explaining the AirTag analogy, CTAs ("View on GitHub", "Back to the protocol" → `Sello.html`), and the interactive **Find My panel** (the centerpiece).
3. **The metaphor** — 4 mapping cards.
4. **The network / key inversion** — "the finder reports the sighting, never the thing being found," with a relay diagram.
5. **Privacy** — ciphertext vs. decrypted, side by side.
6. **CTA** + **Footer**.

## The Find My panel (hero centerpiece) — `.findmy`

A rounded app-like panel (`--bg-2`, `1px --line`, `border-radius: 16px`, big drop shadow) with a header, two tabs, and two switchable views.

- **Header (`.fm-head`):** a small seal mark, a `.tabs` group with two **interactive** buttons — "Agents" (default active) and "Logs" — and a right-aligned "LIVE TRAIL" label with a pulsing `.blip` dot. Active tab gets the green pill (`.tab.on`: accent text, `--accent-dim` border, `--accent-bg` fill). Clicking a tab shows its view and hides the other (`.fm-view[hidden] { display:none }`).

### Agents view (`#view-agents`) — data-driven

- **Agent switcher (`.agent-rail`):** a horizontal row of `.achip` pills (mono, rounded, scrollable). Two agents: `research-agent-01` (active by default) and `ops-agent-02`. Active chip uses the green-pill treatment with an accent status dot. Clicking a chip re-renders the map + sheet for that agent.
- **Map (`.map`, 360px tall; 300px on mobile):** an abstract "radar" — a faint 40px grid (`.mgrid`), three concentric `.ring`s (blue-dim, decreasing opacity) centered at ~46%/52%, a dashed **trail** (`<svg class="trail" viewBox="0 0 100 100" preserveAspectRatio="none">` with a single `<polyline>` using `vector-effect: non-scaling-stroke` and a marching-ants `stroke-dashoffset` animation), and absolutely-positioned **pins** (`.pin`, `left/top` in %). Each pin = a 30px circle (`.pin-dot`, accent border + accent center dot) with a mono `.pin-label` chip below. The most-recent pin gets `.now` (blue instead of green) plus a `.ping` radar pulse ring.
- **Sightings sheet (`.sheet`):** a grab handle, a `.sheet-title` (e.g. "research-agent-01 · 4 verified sightings"), and rows (`.srow`, grid `30px 1fr auto`): a square seal icon, the service + action ("tools/call · result success"), and a right-aligned "✓ verified" + time. The `.now` row uses blue.
- **Data model (in the page's `<script>`):** an `AGENTS` array; each entry = `{ name, trail (polyline points string), pins:[{x,y,label,now?}], sightings:[{svc,time,now?}] }`. `renderAgent(i)` sets the polyline points, rebuilds `#mapPins` and `#sheetRows`, and updates the title. To add agents, push to the array and add a chip. Current data:
  - **research-agent-01** — trail `20,26 73,20 82,62 42,82`; pins maps.api / pay.acme / mail.svc / db.internal(now); 4 sightings 11:02–now.
  - **ops-agent-02** — trail `26,30 70,28 56,78`; pins calendar.api / slack.send / deploy.ci(now); 3 sightings 09:14 / 09:15 / 2m ago.

### Logs view (`#view-logs`)

A transparency-log rendering of the same data: a `.logbar` ("sello.log/example · append-only · 1,042 entries" with pulsing blip), a `.logwrap` table (`.loghead` + `.logrow`s, grid `52px 1fr 1.1fr 40px 52px`) with columns **#** / **sello_token_ref** (blue hash) / **service** / **incl.** (accent ✓) / **time**, the latest row highlighted blue (`.logrow.now`), and a `.logroot` footer showing a faux **Merkle root** hash. On mobile (≤640px) the service column is hidden and the grid tightens.

## Other sections

- **The metaphor (`.maps`):** 2×2 grid of `.mapcard`s. Each card: a muted `.mc-from` line with a blue "Find My" tag + the Find My concept, a large accent `.mc-to` (the Sello equivalent), and a `.mc-desc`. The four mappings: AirTag → sealed receipt · crowdsourced network → services + transparency logs · "last seen" pings → verified sightings · end-to-end encryption → HPKE to the owner.
- **Key inversion (`.inv-grid`):** left, a display `.quote` ("The finder reports the sighting. **Never the thing being found.**") + supporting paragraph; right, a `.relay` card with a 4-node relay diagram (agent → service signs → public log → you verify) using simple glyph icons, the service node in accent and the "you" node in blue, plus a mono caption.
- **Privacy (`.priv-grid`):** two `.pcard`s side by side. Left (`.locked`): "What the network stores" with a `🔒 ciphertext` tag and a blurred, muted base64-ish `.cipher` blob. Right (`.open`): "What your key reveals" with a `✓ decrypted` tag and the decrypted receipt JSON (blue strings, accent `true`). Reinforces end-to-end encryption: the network carries it, only the owner's key reads it.
- **CTA:** eyebrow "It's a metaphor, the protocol is real", H2, paragraph, buttons ("View the repository", "See how the protocol works →" → `Sello.html`).

## Interactions (Find My page)

- **Tab switching** (Agents ⇄ Logs) — toggle `.on` on the clicked tab, show/hide the two `.fm-view`s via the `hidden` attribute.
- **Agent switching** — `renderAgent(i)` rebuilds the map pins, trail polyline, sheet title, and rows from the `AGENTS` data; the clicked `.achip` gets `.on`.
- **Ambient motion** — marching-ants trail dash, radar `.ping` on the live pin, pulsing `.blip` dots, and the shared reveal-on-scroll.
- All the same design tokens, breakpoints, and reveal behavior as the protocol page apply.

## Files (this page)

- `Sello - Find My for Agents.html` — self-contained; CSS in `<head>`, the `AGENTS` data + render/switch logic in the `<script>` at the end of `<body>`.
